...
class AStar:
    ...
    def h_value_tem(self, current_p):
        ...
        # Manhattan distance
        h = (
            np.abs(current_p[0] - self.endPoint[0])
            + np.abs(current_p[1] - self.endPoint[1])
            + np.abs(current_p[2] - self.endPoint[2])
        )
        ...
        for j in range(-1, 2, 1):
            for q in range(-1, 2, 1):
                for w in range(-1, 2, 1):
                    # remove current-point(parent point)
                    if j == 0 and q == 0 and w == 0:
                        continue
                    ...
                    if (
                        x[0] + j < 0
                        or x[0] + j > self.map_grid.shape[0] - 1
                        or x[1] + q < 0
                        or x[1] + q > self.map_grid.shape[1] - 1
                        or x[2] + w < 0
                        or x[2] + w > self.map_grid.shape[2] - 1
                    ):
                        continue
                    ...
                    # adding the child point in open list
    ...
    def judge_location(self, x, j, q, w, list_co):
        ...
        for i in range(list_co.shape[1]):  # print how many columns
            ...
            if (
                x[0] + j == list_co[0, i]
                and x[1] + q == list_co[1, i]
                and x[2] + w == list_co[2, i]
            ):
    ...
    def main(self):
        ...
        while ite <= 2000:
            ...
            if (
                self.current_point[0] == self.endPoint[0]
                and self.current_point[1] == self.endPoint[1]
                and self.current_point[2] == self.endPoint[2]
            ):
